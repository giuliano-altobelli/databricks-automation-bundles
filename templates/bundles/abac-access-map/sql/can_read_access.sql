-- Signature contract:
-- __POLICY_UDF__(principal STRING, __ACCESS_KEY__ STRING) RETURNS BOOLEAN
--
-- The predicate aliases UDF inputs before joining to the access map so
-- column checks remain unambiguous.
CREATE OR REPLACE FUNCTION __POLICY_UDF__(
  principal STRING,
  __ACCESS_KEY__ STRING
)
RETURNS BOOLEAN
RETURN
  CASE
    WHEN principal IS NULL OR __ACCESS_KEY__ IS NULL THEN false
    ELSE EXISTS (
      WITH args AS (
        SELECT
          principal AS requested_principal,
          __ACCESS_KEY__ AS requested_access_key
      )
      SELECT 1
      FROM __ACCESS_MAP_TABLE__ AS access_map
      CROSS JOIN args
      WHERE access_map.effective_principal = args.requested_principal
        AND access_map.__ACCESS_KEY__ = args.requested_access_key
        AND access_map.is_active = true
        AND access_map.access_level IN ('read', 'admin_view')
        AND access_map.valid_from <= current_timestamp()
        AND (
          access_map.expires_at IS NULL
          OR current_timestamp() < access_map.expires_at
        )
    )
  END;
